...
authRouter.post('/register', validateUser, useCode,
...
authRouter.get('/login', passport.authenticate('basic', config.passportOptions),
    async (req: Request, res: Response) => {
        try {
...
authRouter.post('/reset-password', passport.authenticate('jwt', config.passportOptions),
    async (req: Request, res: Response) => {
        try {
